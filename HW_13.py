class GetDataFromFiles:
    ...
    def get_day(self):
        list_day = [date.split(" ")[0].rstrip("nsthrd") for date in self.get_list_date()]
        day_modified_list = []
        for day in list_day:
            if len(day) == 1:
    ...
    def get_months(self):
        months_dict = {"January": "01",
                       ...
                       "November": "11",
                       "December": "12"}
        list_months = [date.split(" ")[1] for date in self.get_list_date()]
        months_modified_list = [months_dict[months] for months in list_months]
        return months_modified_list

    def get_years(self):
        return [date.split(" ")[2] for date in self.get_list_date()]

    def create_list_date_dict(self):
        date_original = [{"date_original": date} for date in self.get_list_date()]
